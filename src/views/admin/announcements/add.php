...
<div class="mt-4">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">เพิ่มประกาศใหม่</li>
        </ol>
    </nav>
    ...
    <div class="row">
        <div class="col-lg-8">
                <div class="card">
                    ...
                    <h5 class="mb-0"><i class="fas fa-plus text-primary"></i> เพิ่มประกาศใหม่</h5>
                    </div>
                    <div class="card-body">
                    <?php if (isset($error) && $error): ?>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="content" name="content" rows="10" 
                                          placeholder="กรอกเนื้อหาประกาศ..." required><?= htmlspecialchars($_POST['content'] ?? '') ?></textarea>
                                <div class="form-text">
                        ...
                        <div class="row mb-3">
                                ...
                                <select class="form-select" id="priority" name="priority">
                                            <option value="low" <?= ($_POST['priority'] ?? 'normal') === 'low' ? 'selected' : '' ?>>
                                        ทั่วไป
                                            </option>
                                            <option value="normal" <?= ($_POST['priority'] ?? 'normal') === 'normal' ? 'selected' : '' ?>>
                                        ปกติ
                                            </option>
                                            <option value="high" <?= ($_POST['priority'] ?? 'normal') === 'high' ? 'selected' : '' ?>>
                                        สำคัญ
                                            </option>
                                            <option value="urgent" <?= ($_POST['priority'] ?? 'normal') === 'urgent' ? 'selected' : '' ?>>
                                        ด่วนมาก
                                            </option>
                                        </select>
                                </div>
                                ...
                                <select class="form-select" id="status" name="status">
                                        ...
                                        ร่าง (ยังไม่เผยแพร่)
                                            ...
                                            <option value="published" <?= ($_POST['status'] ?? 'draft') === 'published' ? 'selected' : '' ?>>
                                        เผยแพร่ทันที
                                            </option>
                                        </select>
                                </div>
            ...
            <!-- Priority Guide -->
                <div class="card">
                    ...
                    <div class="mb-3">
                        ...
                        <small class="text-muted d-block">สำหรับประกาศที่ต้องดำเนินการทันที</small>
                        </div>
                    <div class="mb-3">
                        ...
                        <small class="text-muted d-block">สำหรับประกาศที่สำคัญแต่ไม่เร่งด่วน</small>
                        </div>
                    <div class="mb-3">
                        ...
                        <small class="text-muted d-block">สำหรับประกาศทั่วไป</small>
                        </div>
                    <div>
                        ...
                        <small class="text-muted d-block">สำหรับประกาศที่ไม่สำคัญ</small>
                    </div>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    window.isPageLoad = true; // Set flag for page load
    clearForm();
    setTimeout(() => {
        window.isPageLoad = false; // Reset flag after page load
    }, 100);
});
...
// Reset form change flag when form is submitted or cleared
...
function clearForm() {
    ...
    // If form has content and this is a manual clear (not page load), show confirmation
    if ((title || content || hasFiles || hasEmbedLinks) && !window.isPageLoad) {
        if (!confirm('คุณต้องการล้างฟอร์มทั้งหมดหรือไม่? ข้อมูลที่กรอกไว้จะหายไป')) {
            return;
        }
    }
    ...
    // Show success message for manual clear
...
function processEmbedUrl(url) {
    ...
    if (domain.includes('facebook.com')) {
        return {
            type: 'facebook',
            url: url,
            embedUrl: `https://www.facebook.com/plugins/post.php?href=${encodeURIComponent(url)}&width=500&show_text=true&height=400`,
            title: 'Facebook Post'
        };
    }
    ...
    if (domain.includes('twitter.com') || domain.includes('x.com')) {
        ...
        if (tweetId) {
            return {
                type: 'twitter',
                url: url,
                embedUrl: `https://platform.twitter.com/widgets/tweet.html?id=${tweetId}`,
                title: 'Twitter Post'
            };
        }
    }
    ...
    if (domain.includes('instagram.com')) {
        return {
            type: 'instagram',
            url: url,
            embedUrl: `https://www.instagram.com/p/${extractInstagramId(url)}/embed/`,
...
function updateEmbedPreview() {
    ...
    let html = '<h6 class="mb-3">ลิงก์ที่ฝัง:</h6>';
    
    embedLinks.forEach((link, index) => {
        html += `
            <div class="embed-item mb-3 p-3 border rounded bg-light">
                <div class="d-flex justify-content-between align-items-start mb-2">
                    <div class="d-flex align-items-center">
                        <i class="fas ${getEmbedIcon(link.type)} text-primary me-2"></i>
                        <strong>${link.title}</strong>
                    </div>
                    <button type="button" class="btn btn-sm btn-outline-danger" onclick="removeEmbedLink(${index})">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                <div class="small text-muted mb-2">${link.url}</div>
            ...
            return `
                <div class="text-center">
                    <a href="${link.url}" target="_blank" class="btn btn-outline-primary">
                        <i class="fab fa-twitter me-2"></i>ดูทวีต
                    </a>
                </div>
            `;
        case 'instagram':
...
// Auto-add embed link when Enter is pressed
...
document.getElementById('embedUrl').addEventListener('paste', function(e) {
    setTimeout(() => {
        ...
        if (url && isValidUrl(url)) {
            // Auto-add the pasted URL after a short delay
            setTimeout(() => {
                addEmbedLink();
            }, 500);
        }
...
document.getElementById('image').addEventListener('change', function(e) {
    ...
    if (files.length > 0) {
        ...
        preview.innerHTML = '<h6 class="mb-3">ไฟล์ที่เลือก:</h6>';
...
function previewContent() {
    ...
    let embedContent = '';
    if (embedLinks.length > 0) {
        embedContent = '<div class="mt-4"><h6>ลิงก์ที่ฝัง:</h6>';
        embedLinks.forEach(link => {
            embedContent += generateEmbedContent(link);
        });
        embedContent += '</div>';
    }
