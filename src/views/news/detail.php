...
$images = isset($newsItem['id']) ? $newsModel->getNewsImages($newsItem['id']) : [];
?>
    ...
    <div class="row justify-content-center">
        <div class="col-lg-10 col-xl-8">
            ...
            <div class="card shadow-sm">
                <div class="card-body">
                    <h2 class="fw-bold mb-2" style="color:#b30000; font-size: clamp(1.3rem, 4vw, 2rem);"> <?= htmlspecialchars($newsItem['title']) ?> </h2>
                    <div class="mb-3 text-muted" style="font-size: clamp(0.8rem, 2.5vw, 1rem); border-bottom:1px solid #eee; padding-bottom:6px;">
                        <i class="fas fa-calendar"></i> <?= date('j F Y', strtotime($newsItem['created_at'])) ?>
                        <span class="d-none d-sm-inline">&nbsp;|&nbsp; <i class="fas fa-user"></i> <?= htmlspecialchars($newsItem['author'] ?? 'admin') ?></span>
                    </div>
                    <?php if (!empty($images)): ?>
                        <div class="mb-3 text-center">
                            <img src="<?= htmlspecialchars($images[0]['image_path']) ?>" 
                                 ...
                                 onclick="openImageModal('<?= htmlspecialchars($images[0]['image_path']) ?>', '<?= htmlspecialchars($newsItem['title']) ?>')">
                        </div>
                    <?php endif; ?>
                            ...
                            <?php foreach (array_slice($images, 1) as $img): ?>
                                <div class="col-6 col-sm-6 col-md-4 d-flex justify-content-center">
                                    <img src="<?= htmlspecialchars($img['image_path']) ?>" 
                                         ...
                                         onclick="openImageModal('<?= htmlspecialchars($img['image_path']) ?>', '<?= htmlspecialchars($newsItem['title']) ?>')">
                                </div>
                            <?php endforeach; ?>
                        </div>
                    <?php endif; ?>
                    <div class="mb-3">
                        ...
                        if (!empty($newsItem['category'])) {
                            $categories = array_map('trim', explode(',', $newsItem['category']));
                        }
                        ?>
                    ...
                    <a href="/?page=news" class="btn btn-outline-primary"><i class="fas fa-arrow-left"></i> กลับหน้าข่าวสาร</a>
                </div>
            </div>
        </div>
    </div>
...
/* Mobile responsive adjustments for news detail */
